...
var init = () => {
    ...
    theory.createAutoBuyerUpgrade(2, currency, 1e5);
...

    {
        ...
        c1Exp.description = Localization.getUpgradeIncCustomExpDesc("c_1", "2");
        c1Exp.info = Localization.getUpgradeIncCustomExpInfo("c_1", "2");
...
var getPrimaryEquation = () => {
    ...
    if (c1Exp.level == 1) result += "^{1.07}";
    if (c1Exp.level == 2) result += "^{1.1}";
    if (c1Exp.level == 3) result += "^{1.15}";

    result += "c_2";

    if (c2Exp.level == 1) result += "^{1.07}";
...
var getPublicationMultiplier = (tau) => tau.pow(0.409) / BigNumber.THREE;
var getPublicationMultiplierFormula = (symbol) => "\\frac{{" + symbol + "}^{0.256}}{2.5}";
var getTau = () => currency.value;
var get2DGraphValue = () => currency.value.sign * (BigNumber.ONE + currency.value.abs()).log10().toNumber();

var getC1 = (level) => Utils.getStepwisePowerSum(level, 2, 10, 0);
var getC2 = (level) => BigNumber.TWO.pow(level);
var getC3 = (level) => BigNumber.1000.pow(level);
var getC4 = (level) => BigNumber.5000.pow(level);
